...
/// by `poly`.
/// 
pub fn evaluate_poly<V, W, R>(poly: W, d: usize, point: V, point_ring: &R) -> El<R>
    where V: SelfSubvectorView<El<R>> + Clone,
        W: SelfSubvectorFn<El<R>> + Clone,
        R: RingStore
{
    let m = point.len();
    if m == 0 {
    ...
    assert_eq!(poly.len(), binomial(d + m, m));
    let mut current = point_ring.zero();
    for (i, range) in BlockIter::new(d, m).rev() {
        point_ring.mul_assign_ref(&mut current, point.at(point.len() - 1));
        point_ring.add_assign(&mut current, evaluate_poly(
...
///
/// Iterates over the bounds of a top-level block in the interpolation matrix.
/// More concretely, given variables `x_1, ..., x_m`, we consider all monomials
/// of degree at most `d` in lex-order, so `1 < x_1 < ... < x_1^d < x_2 x_1^(d - 1) < ... < x_m^d`.
...
impl Iterator for BlockIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.current_index < self.back_index {
            let end = self.current_start + binomial(self.d - self.current_index + self.m - 1, self.m - 1);
...
impl DoubleEndedIterator for BlockIter {
    ...
    fn next_back(&mut self) -> Option<Self::Item> {
        if self.back_index > self.current_index {
            self.back_index -= 1;
            let start = self.back_end - binomial(self.d - self.back_index + self.m - 1, self.m - 1);
...
/// in `m` variables of total degree `D` with "unknown" coefficients at all points `(p[1, i_1], ..., p[m, i_m])`
/// for `i_1 + ... + i_m <= D`. Here `p` is an arbitrary, given array, but it will usually be initialized by
/// `p[i, j] = j`.
/// 
        ...
        R: RingStore
{
            ...
            H: Homomorphism<R::Type, S>
    {
        ...
        assert_eq!(out.len(), binomial(out_deg + self.m(), self.m()));
        assert_eq!(input.len(), binomial(in_deg + self.m(), self.m()));
        let mut i = 0;
        let mut j = 0;
        // we use cartesian_product here, despite it yielding some unnecessary elements as
        // the iteration order is already correct
        for tuple in multi_cartesian_product(Iterator::map(0..self.m(), |_| 0..=in_deg), clone_slice) {
    ...
    pub fn add_mul_A<S, H>(&self, d_rows: usize, d_cols: usize, input: &[S::Element], result: &mut [S::Element], factor: S::Element, hom: &H)
        where S: Field,
            H: Homomorphism<R::Type, S>
    {
        debug_assert_eq!(input.len(), binomial(d_cols + self.m(), self.m()));
        debug_assert_eq!(result.len(), binomial(d_rows + self.m(), self.m()));
...
fn test_evaluate_poly() {
    ...
    assert_el_eq!(&ring, &10, &evaluate_poly(SubvectorFn::new((&poly[..]).into_fn()), 3, Subvector::new([1, 1]), &ring));
    assert_el_eq!(&ring, &3, &evaluate_poly(SubvectorFn::new((&poly[..]).into_fn()), 3, Subvector::new([1, 0]), &ring));
    assert_el_eq!(&ring, &6, &evaluate_poly(SubvectorFn::new((&poly[..]).into_fn()), 3, Subvector::new([0, 1]), &ring));
    assert_el_eq!(&ring, &16, &evaluate_poly(SubvectorFn::new((&poly[..]).into_fn()), 3, Subvector::new([1, 2]), &ring));
}